        ...
        <div className="mb-4 md:mb-0 ">
            <ul className="text-[15px] space-y-3">
                {courses?.map((course, index) => (
                    <li className='cursor-pointer flex gap-2 hover:text-green-500 opacity-80 transition-colors duration-500' key={index}>
                        < PiDotOutlineThin className='text-white mt-1 ' />{course}</li>
                ))}
            </ul>
        </div>
    );
        ...
        <footer
            ...
            <div className="text-center text-sm mt-4">
                ...
                <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer">  CodeInnovior .</a>
            </div>
        </footer>
    );
};
